...
\begin{tabular}{|l|c|c|c|c|}
    ...
    \hline
    \mips{lw f2, 0(r1)} & \az{1} & \az{2 - 3} & \az{4} & \az{5} \\
    \hline
    \mips{lw f4, 0(r2)} & \az{2} & \az{3 - 4} & \az{5} & \az{6} \\
    \hline
    \mips{mul f6, f2, f4} & \az{3} & \az{5 - 8} & \az{9} & \az{10} \\
    \hline
    \mips{add f2, f6, f4} & \az{4} & \az{9 - 10} & \az{11} & \az{12} \\
    \hline
    \mips{add f8, f2, f4} & \az{5} & \az{11 - 12} & \az{13} & \az{14} \\
    \hline
    \mips{mul f10, f8, f2} & \az{6} & \az{13 - 16} & \az{17} & \az{18} \\
    \hline
    \mips{add f6, f2, f8} & \az{7} & \az{13 - 14} & \az{15} & \az{19} \\
    \hline
    \mips{add f12, f6, f10} & \az{8} & \az{17-18} & \az{19} & \az{20} \\
    \hline
\end{tabular}
\end{figure}
...
\renewcommand{\thefigure}{\arabic{figure}} % restore default afterward
    ...
    { \color{azul}  
        ...
        Esta política es coherente con \textbf{write-back}, ya que al traer el bloque al caché se aprovecha la localidad espacial y temporal. \\
...
int main() 
{
    ...
    \\ { \color{azul} 
        ...
        es probable que pronto se acceda a direcciones cercanas.
...
\item Se tiene un sistema con direcciones de memoria de 32 bits y un caché de 8KB con bloques de 32 bytes. \par
    ...
    \\ { \color{azul} 
        En el \textbf{mapeo directo}, cada bloque de memoria solo puede ubicarse en un único bloque del caché,
        Si dos bloques frecuentemente usados mapean al mismo índice, se producen conflictos de caché: 
        cada vez que se accede a uno, el otro es desalojado, generando miss tras miss (trashing).
        Esto ocurre aunque el 99\% del caché esté vacío. \par
